...
export async function GET(
  request: NextRequest,
  { params }: { params: { callId: string; stageId: string } }
) {
  try {
    const { callId, stageId } = params;
    ...
    const response = await fetch(`https://api.ultravox.ai/api/calls/${callId}/stages/${stageId}`, {
      method: 'GET',
      headers: {
        'X-API-Key': apiKey
      }
    ...
    console.error('Server error getting call stage:', error);
    return NextResponse.json(
